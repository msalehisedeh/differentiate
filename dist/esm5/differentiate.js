...
var DifferentiateComponent = /** @class */ (function () {
    ...
    DifferentiateComponent.prototype.performAdvanceToRight = function (leftSideInfo, rightSideInfo, status, index) {
        var _this = this;
        ...
        setTimeout(function () {
            _this.onadvance.emit(_this.transformNodeToOriginalStructure(_this.leftSide[0].children[index].children, DifferentiateNodeType.json));
            _this.fireCountDifference();
        }, 66);
    };
    DifferentiateComponent.prototype.performAdvanceToLeft = function (leftSideInfo, rightSideInfo, status, index) {
        var _this = this;
        ...
        setTimeout(function () {
            _this.onrevert.emit(_this.transformNodeToOriginalStructure(_this.rightSide[0].children[index].children, DifferentiateNodeType.json));
            _this.fireCountDifference();
        }, 66);
    };
    DifferentiateComponent.prototype.advance = function (event) {
        var path = event.node.path.split(",");
        if (event.type === 'advance') {
            this.performAdvanceToLeft(this.lookupChildOf(this.leftSide[0].children[parseInt(path[1])], event.node.id), this.lookupChildOf(this.rightSide[0].children[parseInt(path[1])], event.node.counterpart), event.node.status, parseInt(path[1]));
        }
        else {
            this.performAdvanceToRight(this.lookupChildOf(this.leftSide[0].children[parseInt(path[1])], event.node.counterpart), this.lookupChildOf(this.rightSide[0].children[parseInt(path[1])], event.node.id), event.node.status, parseInt(path[1]));
        }
    };
    DifferentiateComponent.prototype.autoExpand = function (event) {
        var path = event.split(",");
        var lc = this.rightSide[0].children[parseInt(path[1])];
        var rc = this.leftSide[0].children[parseInt(path[1])];
    ...
    DifferentiateComponent.prototype.onhover = function (event) {
        ...
        var lc = this.rightSide[0].children[parseInt(path[1])].children;
        var rc = this.leftSide[0].children[parseInt(path[1])].children;
